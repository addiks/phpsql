...
class BTree implements \IteratorAggregate, IndexInterface
{
    ...
    {
        $this->removeRow($oldRow, $rowId);
        $this->insertRow($newRow, $rowId);
    }
    ...
    public function searchRow(array $row)
    {
        ...
        /* @var $indexPage IndexSchema */
        $indexPage = $this->getIndexSchema();

        foreach ($this->indexPage->getColumns() as $columnId) {
            /* @var $columnPage ColumnSchema */
            $columnPage = $this->tableSchema->getColumn($columnId);
    ...
    public function insertRow(array $row, $rowId)
    {
        ...
        $rowIdPadded = str_pad($rowId, $completeKeyLength, "\0", STR_PAD_LEFT);

        $this->insert($useValue, $rowIdPadded);
    }
    ...
    public function removeRow(array $row, $rowId)
    {
        ...
        foreach ($this->indexPage->getColumns() as $columnId) {
            ...
            if (isset($row[$columnId])) {
                $valueCell = $row[$columnId];
            } elseif (isset($row[$columnPage->getName()])) {
                $valueCell = $row[$columnPage->getName()];
            }
            $keyLength = $columnPage->getCellSize();
            $completeKeyLength += $keyLength;
            $useValue .= str_pad($valueCell, $keyLength, "\0", STR_PAD_LEFT);
    ...
    public function getIterator($beginValue = null, $endValue = null)
    {
        ...
        if (is_int($beginValue)) {
            ...
            foreach ($this->indexPage->getColumns() as $columnId) {
                ...
                $keyLength = $columnPage->getCellSize();
                $beginValue .= str_pad($valueCell, $keyLength, "\0", STR_PAD_LEFT);
            }
        }
        ...
        if (is_int($endValue)) {
            ...
            foreach ($this->indexPage->getColumns() as $columnId) {
                ...
                $keyLength = $columnPage->getCellSize();
                $endValue .= str_pad($valueCell, $keyLength, "\0", STR_PAD_LEFT);
            }
        }
        ...
        return new CustomIterator(null, [
            ...
            'current' => function () use (&$rowId, $btree) {
                $btree->stringDecrement($rowId);
                return $rowId;
            },
            'key' => function () use (&$value) {
                $btree->stringDecrement($value);
                return $value;
            },
    ...
    public function decrementIteratorPath(&$path, &$value, &$rowId, &$nodePath = null, &$nodePathKeys = null)
    {
        ...
        if (count($path)<=0) {
            throw new ErrorException("Empty iterator-path given to decrement!");
        }
