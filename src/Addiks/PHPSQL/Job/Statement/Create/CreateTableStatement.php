...
use Addiks\PHPSQL\Job\Part\Join\Table as TablePart;
...
use Addiks\PHPSQL\Job\Part\Index;
use Addiks\PHPSQL\Job\Statement\Select;
...
class CreateTableStatement extends CreateStatement
{
    ...
    public function setMaximumRows($int)
    {
        $int = (int)$int;
        if ($int < 0) {
            throw new MalformedSqlException("Maximum-rows '{$int}' cannot be negative!");
        }
    ...
    public function setMinimumRows($int)
    {
        $int = (int)$int;
        if ($int < 0) {
            throw new MalformedSqlException("Minimum-rows '{$int}' cannot be negative!");
        }
