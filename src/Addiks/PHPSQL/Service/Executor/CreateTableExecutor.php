...
use Addiks\Database\Value\Enum\Page\Index\ForeignKeyMethod;
...
use Addiks\Database\Value\Enum\Page\Index\Engine as IndexEngine;

use Addiks\Database\Value\Enum\Page\Schema\Type;
use Addiks\Database\Value\Enum\Page\Index\Type as IndexType;
...
use Addiks\Database\Resource\Index as IndexResource;
		...
		switch(true){
			...
			case(is_array($statement->getColumnDefinition())):
				foreach($statement->getColumnDefinition() as $name => $column){
					...
					foreach($metadata->getAnnotationsByClassConstant(get_class($column->getDataType()), $dataType->getName()) as $annotations){
						foreach($annotations as $annotation){
							/* @var $annotation Annotation */
					
							switch($annotation->getIdentifier()){
								case 'Addiks\\\\Datatype':
									if(isset($annotation['bytelength'])){
										$columnPage->setLength($annotation['bytelength']);
									}
									if(isset($annotation['secondbytelength'])){
										$columnPage->setSecondLength($annotation['secondbytelength']);
									}
									break;
							}
						}
