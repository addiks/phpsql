...
class CustomIterator extends \IteratorIterator implements \ArrayAccess{
	...
	public function __construct($iterator = null, $lambdas=array()){
		...
		foreach($lambdas as $key => $lambda){
			switch($key){
				case 'rewind':	$this->setRewindCallback($lambda);  break;
				case 'valid':	$this->setValidCallback($lambda);   break;
				case 'current':	$this->setCurrentCallback($lambda); break;
				case 'key':		$this->setKeyCallback($lambda);     break;
				case 'next':	$this->setNextCallback($lambda);    break;
				case 'seek':	$this->setSeekCallback($lambda);    break;
				case 'set':     $this->setSeekCallback($lambda);    break;
				case 'get':     $this->setGetCallback($lambda);     break;
				case 'isset':   $this->setIssetCallback($lambda);   break;
				case 'unset':   $this->setUnsetCallback($lambda);   break;
			}
		}
	...
	public function next(){
		if(is_callable($this->nextCallback)){
			return call_user_func($this->nextCallback, function(){parent::next();});
		}else{
			return parent::next();
		}
	}
	...
	public function rewind(){
		if(is_callable($this->rewindCallback)){
			return call_user_func($this->rewindCallback, function(){parent::rewind();});
		}else{
	...
	public function seek($rowId){
		if(is_callable($this->seekCallback)){
			$seekClosure = function() use($rowId){parent::seek($rowId);};
