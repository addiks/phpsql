...
namespace Addiks\Database\Tool;
...
#		$statementString = preg_replace("/\s+/is", " ", $statementString);
		...
		$reflectionTokenEnum = new \ReflectionClass("\Addiks\Database\Value\Enum\Sql\SqlToken");
		...
		while(strlen($statementStringLeft)>0){
			...
			#		break;
				...
				case preg_match("/{$tokenNamePattern}/is", $statementStringLeft, $matches):
					...
					$position += strlen($matches[1]);
					$tokenValue = call_user_func("\Addiks\Database\Value\Enum\Sql\SqlToken::{$tokenName}");
